...
# TO DO
# in the result table our/their add in the last table all mismatches. by creating a string where it's Local_fail and adding it
# add colouring the cells when local_fail in their
# check whether one or more key is returned in their table
# check empty value returned scenario
    ...
    if row_cell[0].value is not None:
        ...
        if key_value_tuple is not None and key_value_tuple[0] == True:
            ...
            for cell in row_cell[1:]:
                ...
                # find value in following column and in above row
                logging.debug(str(mapping_tuple[column_index-1][1]))
